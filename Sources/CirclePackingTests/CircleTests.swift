...
class CircleTests: XCTestCase {
    ...
    func testSharedDiameterCollisionPoints() {
        let a = FlatCircle(radius: 10, center: Point(x: 1, y: 2))
        let b = FlatCircle(radius: 10, center: Point(x: 1, y: 20))
        let points = a.sharedDiameterCollisionPoints(b)
        XCTAssertTrue(points.contains(Point(x: 1, y: -8)))
        XCTAssertTrue(points.contains(Point(x: 1, y: 10)))
        XCTAssertTrue(points.contains(Point(x: 1, y: 12)))
        XCTAssertTrue(points.contains(Point(x: 1, y: 30)))
    }

    func testDiameterCollisionPoints() {
        let c = FlatCircle(radius: 10, center: Point(x: 1, y: 2))
        let points = c.diameterCollisionPoints(passing: Point(x: 1, y: 200))
        XCTAssertEqual(points.first, Point(x: 1, y: -8))
        XCTAssertEqual(points.second, Point(x: 1, y: 12))
    }

    func testRelativeLocation() {
        let a = FlatCircle(radius: 2, center: .zero)
